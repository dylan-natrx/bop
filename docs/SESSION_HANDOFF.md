...
Last meaningful work: **2026-07-15 — BOP password gate removed (pre-publication) + `noindex` until launch (`1a8b9a0`). Immediately prior: the 2026-07-13/14 platform restructure (`aeca745`…`13ef5eb`) converting this repo into the natrx.report multi-project platform, and the 2026-07-13 BOP client review round (`6301891`, `3b2d007`, `1a4daf8`, `5ca05b9`). Read the platform section and the review-round section below first.**

Prior state: 2026-05-26. **v1 is live at https://bop.natrx.report.** All five sections have real copy and real visuals. Map storytelling rebuilt: biology gates (steps 1–3), external factors overlay as flag markers (steps 4–6), priority projects revealed at step 6 with a pulsing halo. Spectra panel uses a two-color treatment (teal = biology, amber = external). Legend evolved into a horizontal strip pinned to the bottom of the map. Mobile section nav got a hamburger menu. Custom domain wired up with Cloudflare DNS + per-domain Vercel CNAME target. Three pullquotes (Carolyn Khoury §2, Mike McCann §3, Lise Montefiore PhD §5). Vercel Analytics + Speed Insights are live with custom editorial events.

---

## The natrx.report platform (2026-07-13/14) — READ THIS FIRST

This repo is no longer just the BOP site. It was converted **in place** into a multi-project platform hosting independently designed report sites at `[project].natrx.report`, per `natrx-report-platform-packet-v2.md` (repo root). Eight commits, `aeca745` through `13ef5eb`, verified at every phase boundary with the regression harness in `harness/` (Playwright content + pixel diff against a committed production baseline; see `harness/README.md`).

**The governing principle: the platform owns infrastructure, projects own design.** No shared component library, no theming system, no cross-project imports (enforced by `eslint-rules/no-cross-project-imports.mjs`, not convention).

- **`src/middleware.ts`** — hostname → tenant → gate → rewrite. `bop.natrx.report/x` rewrites to `/projects/bop/x`; the public URL never shows `/projects/`. The apex `natrx.report` 302s (never 301) to `https://natrx.io`. Unknown subdomains 404. Localhost/preview traffic falls back to `?tenant=` param, then `PLATFORM_DEFAULT_TENANT`, then `bop`.
- **`src/lib/platform/tenants.ts`** — the registry: slug, displayName, `accessMode` (`draft`/`gated`/`public`), bcrypt `passwordHash`, `mediaUrl`. No design fields, deliberately. Adding a project = one directory under `src/app/projects/`, one registry entry, one DNS record.
- **`src/lib/platform/gate.ts`** — the access gate, modeled on `evaluateShareGate` in the unified app so the eventual fold is a swap. Session is an httpOnly host-only cookie `nx-report-<slug>` whose value is `HMAC-SHA256(PLATFORM_SESSION_SECRET, "<slug>:<passwordHash>")`. Rotating the tenant password or the secret invalidates that tenant's sessions. Fails closed if the secret is missing. Edge-safe (Web Crypto only); bcrypt lives in `credentials.ts`, imported only by the Node-runtime login route.
- **`/media`** — ungated 302 (never 301/308; browsers and Google cache permanent redirects forever) to the tenant's `mediaUrl`. BOP's points at `https://share.natrx.io/billion-oyster-project`. Null `mediaUrl` → 404.
- **Tenants:** `bop` (**public** as of 2026-07-15, see below) and `demo` (`demo.natrx.report`, gated, deliberately un-BOP-looking; exists to prove isolation).
- **Root layout** (`src/app/layout.tsx`) is a bare shell: html, body, `<Analytics />`, `<SpeedInsights />`. Everything BOP (fonts, metadata, OG image, `styles/bop.css`) lives in `src/app/projects/bop/layout.tsx`. `src/app/globals.css` is Tailwind directives only.
- **Env:** `PLATFORM_SESSION_SECRET` (Vercel + `.env.local`) and `NEXT_PUBLIC_MAPBOX_TOKEN`. The old `AUTH_USERNAME` / `AUTH_PASSWORD` / `AUTH_TOKEN` / `AUTH_DISABLED` vars are **gone**.
- **Deploy note:** the restructure was deployed to production via `vercel` CLI on 2026-07-13; GitHub `main` was synced two days later (with the gate removal). Watch for the two-account push issue (below).
...
To re-gate BOP: set `accessMode: 'gated'` and put a bcrypt hash of `"<username>:<password>"` in `passwordHash`. The demo tenant's credential is separate; its cookie grants nothing on bop and vice versa (host-only cookies).

---

## BOP client review round (2026-07-13) — READ THIS SECOND
...
6. ~~Dot-size legend on the walkthrough map.~~ **Closed.** McCann's comment was anchored to Map 2 (the step-1 walkthrough map), confirmed from his screenshot. `WalkthroughMapLegend` now carries a `SizeKey` entry reading "Dot size = site acreage", drawn at the map's true radius extremes (3px min / 8px at the 470-acre max). The hero map got the same answer independently (`AreaLegend` in `FigurePanel` + a rewritten Fig. 1 caption). Both maps now explain the encoding.
7. **Eyes on the new `AreaLegend`.** It was never seen rendered (the 2026-07-13 page capture caught the hero mid-animation and the whole `FigurePanel` was invisible). The circles are drawn at the map's true radii, so they're small: 7px to 16px diameter. Faithful, possibly too subtle to be useful. Check it, and scale the whole ramp up if it doesn't read.
...
1. **The client sign-offs still open from the 2026-07-13 review round** (see "Still open" in that section): McCann on the Superfund phrasing, confirmation of the 2030 goal date, Khoury on the § 3 connectivity headline.
2. **At launch: remove the `noindex`** from `src/app/projects/bop/layout.tsx` (checklist in `NOTES.md`).
3. **Mobile QA pass.** All sections have been built lg-first. Real mobile verification still pending.
4. **Optional follow-up:** ask Nick Brady for the shoreline-change point data so the § 4 image can become web-native and dark.
5. **Optional follow-up:** the "Applied Filters" panel in the methodology walkthrough should be re-passed to read as a confident running summary, not a glossary of definitions. See `memory/project_applied_filters_panel_rewrite.md`.
6. **Optional follow-up:** delete the dead-code paths flagged in `NOTES.md` — `src/app/projects/bop/components/hero/CoastlineTest.tsx`, `src/app/projects/bop/test-map/`, `src/app/projects/bop/site/[siteId]/`, `src/app/projects/bop/lib/land.ts` (legacy SVG coastline), most of `src/app/projects/bop/lib/projection.ts` (only `calculateMarkerRadius` is still used). And once those are gone, delete the now-unused `public/data/nyc-boroughs.geojson` / `nj-shoreline.geojson` / `westchester.geojson` / `upstate-ny-ct.geojson` files (production no longer references them).
