...
6. **Dot-size legend on the walkthrough map (Map 2).** McCann: "It's unclear what the size of the dots corresponds to in this map." That was answered on the **hero map only** (new `AreaLegend` in `FigurePanel`, plus a rewritten Fig. 1 caption naming acreage outright). But `WalkthroughMap` sizes its dots by acreage on the same `_radius` property, and its legend strip only carries "Suitable site / Below threshold." If he was looking at Map 2, the note is still open. Add a size key to the walkthrough legend strip, or confirm with him which map he meant.
